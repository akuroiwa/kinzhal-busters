...
export class PSO {
  ...
  evaluate(position, missiles, powerPlant, activeDroneCount, totalDroneCount) {
    let fitness = 0;
    ...
    // Drone survival ratio (want to maximize)
    const survivalRatio = activeDroneCount / totalDroneCount;
    fitness -= survivalRatio * 0.5; // Lower weight to prioritize existing conditions

    return fitness;
  }

  optimize(drones, missiles, powerPlant) {
    const activeDroneCount = drones.filter(drone => !drone.isDestroyed).length;
    const totalDroneCount = drones.length;
    ...
    this.particles.forEach((particle, i) => {
      ...
      // Evaluate fitness
      const fitness = this.evaluate(particle.position, missiles, powerPlant, activeDroneCount, totalDroneCount);
      ...
      // Update global best
      if (!this.globalBest || fitness < this.evaluate(this.globalBest, missiles, powerPlant, activeDroneCount, totalDroneCount)) {
